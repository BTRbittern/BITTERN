...
pkgdatadir = $(datadir)/bittern
pkgincludedir = $(includedir)/bittern
pkglibdir = $(libdir)/bittern
pkglibexecdir = $(libexecdir)/bittern
...
CONFIG_HEADER = $(top_builddir)/src/config/bittern-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/bittern-config.h.in \
	$(top_srcdir)/test/config.ini.in COPYING INSTALL \
...
ACLOCAL = ${SHELL} /prj/coinFactory/works/bitcoin/bittern/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /prj/coinFactory/works/bitcoin/bittern/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /prj/coinFactory/works/bitcoin/bittern/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /prj/coinFactory/works/bitcoin/bittern/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = bittern-cli
BITCOIN_DAEMON_NAME = bitternd
BITCOIN_GUI_NAME = bittern-qt
BITCOIN_TX_NAME = bittern-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /prj/coinFactory/works/bitcoin/bittern/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = bittern
PACKAGE_BUGREPORT = bittern.io
PACKAGE_NAME = Bittern Core
PACKAGE_STRING = Bittern Core 1.0.0
PACKAGE_TARNAME = bittern
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /prj/coinFactory/works/bitcoin/bittern
abs_srcdir = /prj/coinFactory/works/bitcoin/bittern
abs_top_builddir = /prj/coinFactory/works/bitcoin/bittern
abs_top_srcdir = /prj/coinFactory/works/bitcoin/bittern
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /prj/coinFactory/works/bitcoin/bittern/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/bittern-cli.bash-completion \
	       $(top_srcdir)/contrib/bittern-tx.bash-completion \
	       $(top_srcdir)/contrib/bitternd.bash-completion \
...
COVERAGE_INFO = baseline.info \
  test_bittern_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_bittern_coverage.info test_bittern.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Bittern-Core $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
EXTRA_DIST = $(DIST_SHARE) qa/pull-tester/rpc-tests.sh \
	qa/pull-tester/run-bitcoin-cli qa/rpc-tests $(DIST_CONTRIB) \
...
src/config/bittern-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bittern-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bittern-config.h
$(top_srcdir)/src/config/bittern-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bittern-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Bittern-Core: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Bittern-Core
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Bittern-Core: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_bittern.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_bittern -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_bittern_filtered.info: test_bittern.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_bittern_filtered.info
...
#test_bittern_coverage.info: baseline_filtered.info test_bittern_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_bittern_filtered.info -o $@

#total_coverage.info: test_bittern_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_bittern_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bittern.coverage/.dirstamp:  test_bittern_coverage.info
...
#cov: test_bittern.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_bittern.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
