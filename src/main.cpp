...
#if defined(NDEBUG)
#error "BTR cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubittern) are considered zero fee (for relaying and mining)
...
{
    RenameThread("bittern-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateBTRSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BTR: We use certain sporks during IBD, so check to see if they are
