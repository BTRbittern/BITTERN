...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BTR);
    ...
    if (!settings.contains("nAnonymizeBTRAmount"))
        settings.setValue("nAnonymizeBTRAmount", 1000);

    nAnonymizeBTRAmount = settings.value("nAnonymizeBTRAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBTRAmount"))
        SoftSetArg("-anonymizebitternamount", settings.value("nAnonymizeBTRAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bittern.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBTRAmount:
            return QVariant(nAnonymizeBTRAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBTRAmount:
            nAnonymizeBTRAmount = value.toInt();
            settings.setValue("nAnonymizeBTRAmount", nAnonymizeBTRAmount);
            emit anonymizeBTRAmountChanged(nAnonymizeBTRAmount);
            break;
        case CoinControlFeatures:
